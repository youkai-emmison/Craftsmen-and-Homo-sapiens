# 《能工智人》评委单页摘要

## 30 秒看懂

《能工智人 / Craftsmen and Homo sapiens》是一个 Unity 2D 横版动作地牢 Demo，参赛方向为腾讯云黑客松赛题三“叙事类游戏 / AI 重塑叙事体验”。玩家进入由 AI 档案重构的地下工坊，在移动、跳跃、战斗、成长和 Boss 挑战中逐步理解“工匠”与“智人”分裂的真相。

## 核心演示流程

```text
开场 NPC / 档案提示
-> Early Room：熟悉移动、跳跃、攻击
-> 击败小怪，获得成长反馈
-> Mid Room：面对更强敌人，体现成长后战斗更快
-> Boss Room：击败 Demo Boss
-> Victory / Demo Complete
```

## AI 用在什么地方

- AI 叙事创作：生成地下工坊世界观、房间档案、NPC 台词、敌人和 Boss 设定。
- AI 美术辅助：规划主角、怪物、道具、地图、UI 等占位视觉方向。
- AI 编程协作：使用 CodeBuddy / AI 编程助手辅助 Unity C# 脚本、调试、文档、PPT 和提交材料整理。

## 评审看点

| 评分方向 | 本项目对应内容 |
| --- | --- |
| 主题契合度 | AI 档案叙事进入关卡流程，玩家边战斗边理解世界观 |
| AI 工具使用 | AI 参与叙事、美术规划、代码协作和提交材料整理 |
| 游戏品质 | 有移动、战斗、成长、清房、Boss、胜利反馈的完整短闭环 |

## 快速试玩信息

- WebGL 在线试玩：https://craftsmen-and-homo-sapiens.onrender.com
- Demo 视频：待回填
- GitHub 仓库：https://github.com/youkai-emmison/Craftsmen-and-Homo-sapiens/tree/master
- 推荐体验时长：3 到 5 分钟

## 操作方式

- 移动：`A / D`
- 跳跃：`Space`
- 攻击：`J` 或鼠标左键
- 对话 / 交互：`E`
- 背包：`I`

## 提交材料位置

- 最终交付包清单：`submissions/FINAL_DELIVERABLES_MANIFEST.md`
- 项目书 PDF：`submissions/PROJECT_BOOK_FINAL_ZH.pdf`
- 作品介绍 PPT：`submissions/Craftsmen_Hackathon_Deck.pptx`
- 问卷复制稿：`submissions/FORM_ANSWERS_COPYPASTE_ZH.md`
- 评委快速打开指南：`submissions/JUDGE_QUICK_START.md`
