...
当前 `MainScene` 已经保存了一版可演示灰盒：
- `StageBlockout` 已生成，包含 `Background`、`Ground`、`Platforms`、`Boundaries`、`Markers`。
- `StageBlockout` 现在也包含 `Decorations`，用于柔和区分出生区、训练区、战斗区。
- `Player` 下已有 `PlayerHeroineVisual`，使用 `Assets/Art/Generated/Characters/heroine_placeholder.png`。
- `Player` 下已有 `PlayerHealthBar`，用于显示当前血量。
- `Player` 下已有 `AttackSlashVisual`，按 J 或鼠标左键攻击时会短暂显示斩击。
- `PlayerFacingController` 会根据左右输入翻转角色，并把 `attackPoint` 移到左侧或右侧。
- `StageBlockoutBuilder` 使用 `Assets/Art/Generated/Environment/blockout_square.png` 作为灰盒方块 Sprite，避免依赖 Unity 内置资源路径。
- `MainGround` 是贯穿关卡的长地板，用来避免 Player 往左右移动就掉出短地板。

如果 Unity 里仍然看不到角色，先退出 Play 模式，然后重新打开 `Assets/Scenes/MainScene.unity` 或在 Project 面板右键 Refresh。
